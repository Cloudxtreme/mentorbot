    ...
    public sealed class CalendarProcessor : ICommandProcessor
    {
        ...
        public async ValueTask<ChatEventResult> ProcessCommandAsync(TextDeconstructionInformation info, ChatEvent originalChatEvent, IAsyncResponder responder)
        {
            ...
            var keyValue = new KeyValue
            {
                ...
                Icon = "INVITE",
                Button = link == null ? null : CreateTextButton(link.Label, link.Uri)
            };

            return new ChatEventResult(CreateCardFromWidget(keyValue));
        }

        // TODO: Extact in extensions.
        private static Card CreateCardFromWidget(KeyValue keyValue) =>
            new Card
            {
                Sections =
                {
        ...
        private static Button CreateTextButton(string label, string url) =>
             new Button
             {
